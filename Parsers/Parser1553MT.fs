...
module Parser1553MT =
    ...
    let SetElementDataRecord1553MT (aList:string list) = 
        let currentElement = 
            {
                MsgNO   = (filterByParameter aList "MsgNO" ) |> int
                ...
                status1 = outputValue (filterByParameter aList "status1")
                status2 = outputValue (filterByParameter aList "status2")
                ...
                words   = (aList.[10..(aList.Length - 1)] 
                          ...
                          |> Seq.map (fun x -> Convert.ToInt32(x, 16))
                          |> Seq.toList
